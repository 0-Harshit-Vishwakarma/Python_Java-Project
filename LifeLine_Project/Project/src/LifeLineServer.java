...
public class LifeLineServer {
    ...
    private static void deleteAccount(HttpExchange exchange) throws IOException {
        ...
        synchronized (LifeLineServer.class) {
            ...
            // Prefer the user ID, but use the email as a safe fallback if an
            // older browser has stale localStorage without a user ID.
            if (userId.isBlank() && !email.isBlank()) {
                for (Map<String, String> user : users) {
                    if (email.equalsIgnoreCase(user.getOrDefault("email", ""))) {
                        userId = user.getOrDefault("id", "");
                        break;
                    }
                }
            }
            ...
            boolean removed = users.removeIf(user ->
                    targetUserId.equals(user.getOrDefault("id", "")));
            ...
            cards.removeIf(card -> targetUserId.equals(card.getOrDefault("userId", "")));
            writeJsonArray(CARDS_FILE, cards);

            // Verify that the account and its emergency card are actually gone
            // before reporting success to the browser.
            boolean userStillExists = readJsonArray(USERS_FILE).stream()
                    .anyMatch(user -> targetUserId.equals(user.getOrDefault("id", "")));
            boolean cardStillExists = readJsonArray(CARDS_FILE).stream()
                    .anyMatch(card -> targetUserId.equals(card.getOrDefault("userId", "")));

            if (userStillExists || cardStillExists) {
                sendJson(exchange, 500,
                        "{\"success\":false,\"message\":\"Account could not be completely deleted.\"}");
                return;
            }
